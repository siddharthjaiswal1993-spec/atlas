...
export function generateRecommendations(clientId: string): Recommendation[] {
  ...
  return [
    ...
    },
    {
      ...
      confidence: 'High',
      impact: 'High',
      suggestedOwner: 'Content Strategist',
      status: 'pending',
      addedToEmail: false,
    ...
    },
    {
      ...
      confidence: 'Medium',
      impact: 'High',
      suggestedOwner: 'SEO/GEO Strategist',
      status: 'pending',
      addedToEmail: false,
      category: 'AI/GEO Visibility',
    },
    {
      id: 'rec-4',
    ...
    {
      id: 'rec-7',
      title: `Investigate ${Math.abs(Math.round(loser?.change?.replace(/[^-\d]/g, '') as unknown as number))} click drop on "${loser?.query}"`,
      ...
      confidence: 'Medium',
      impact: 'Medium',
      suggestedOwner: 'SEO Analyst',
      status: 'pending',
      addedToEmail: false,
      category: 'Traffic Investigation',
    },
    {
      id: 'rec-8',
...
export function generateReport(clientId: string): GeneratedReport {
  ...
  const risks = [
    {
      title: `Traffic drop: "${gsc.losers[0]?.query}"`,
      description: `${gsc.losers[0]?.change} in clicks vs prior period. Likely caused by competitor rank gain and possible SERP feature displacement.`,
      severity: 'high' as const,
      source: 'GSC',
    },
    {
      title: `Ranking decline: ${cms.losingRankPages[0]?.title}`,
      description: `Position deteriorated by +${cms.losingRankPages[0]?.positionChange?.toFixed(1)} positions (now at ${cms.losingRankPages[0]?.currentPosition?.toFixed(1)}). Page may need content refresh.`,
      severity: 'medium' as const,
      source: 'GSC + Semrush',
    },
    {
      title: 'Competitor AI mention surge',
      description: `${semrush.competitors[0]?.name} visibility score on Perplexity is ${geo.competitors[0]?.visibilityScore}/100 vs ${client?.name}'s ${geo.engines.find(e => e.name === 'Perplexity')?.visibilityScore}/100. Gap is widening.`,
      severity: 'high' as const,
      source: 'GEO/AI',
    },
    {
      title: `${cms.stalePages.length} CMS pages not updated in 90+ days`,
      description: `Stale pages risk ranking decay. Top affected: "${cms.stalePages[0]?.title}" (${cms.stalePages[0]?.daysStale} days stale, ${cms.stalePages[0]?.traffic?.toLocaleString()} monthly visits).`,
      severity: 'medium' as const,
      source: 'CMS',
    },
    {
      title: 'Missing AI citations in Google AI Overviews',
      description: `${client?.name} not cited in Google AI Overviews responses for ${geo.topPrompts.filter(p => !p.mentioned).length} tracked prompts. Competitors dominate these responses.`,
      severity: 'medium' as const,
      source: 'GEO/AI',
    },
  ];
...
export function calculateEvalMetrics(): EvalMetrics {
  return {
    ...
    aiDraftMinutes: 8,
    reviewMinutes: 20,
    totalAiAssistedMinutes: 28,
    timeSavedPercent: 88,
    completenessScore: 94,
    evidenceBackedPercent: 96,
    unsupportedClaimCount: 2,
    recommendationUsefulnessScore: 4.4,
    csEditEffortScore: 18,
    approvalReadinessScore: 91,
  };
}
